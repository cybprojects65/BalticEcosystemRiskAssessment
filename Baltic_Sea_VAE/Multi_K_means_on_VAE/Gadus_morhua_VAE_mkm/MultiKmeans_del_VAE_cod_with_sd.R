...
variables_risk1_standardized <- read_csv("output_vae_cod_highrisk.csv")
...
multi_centroidi<-  seq(from=3, to=60, by=1)  #number of centrodi from 3 to 60 or enter c(4) to make it with just one number
...
########################################################################  
  ...
  write.csv(centroidi_annotated, 
            file = paste0("./output_sd/all_centroidi_annotated_", n_centroidi, "_", year, "_cod_mkm_vae_sd.csv"), 
            row.names = FALSE)
  ...
  output_file<-paste0("./output_sd/all_centroid_classification_assignment_",n_centroidi,"_", year, "_cod_mkm_vae.csv")
